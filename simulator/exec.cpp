...
void exec_update_destination_floor()
{
    ...
    if (inside_queue->length) destination_floor = static_cast<floor_codes_t>(inside_queue->queue[0].floor);
    else if (outside_queue->length) destination_floor = static_cast<floor_codes_t>(outside_queue->queue[0].floor);
